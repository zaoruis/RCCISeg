...
class MaskFormer(nn.Module):
    ...
    def count_parameters(self, print_info: bool = True) -> int:
        ...
        if print_info:
            print("=" * 60)
            print(f"{'MaskFormer模型参数量统计':^60}")
            print("=" * 60)
            print(f"{'总参数量':<20}{total_params:,}{'':<10}{f'{total_params / 1e6:.2f}M':>20}")
            print(f"{'可训练参数量':<20}{trainable_params:,}{'':<10}{f'{trainable_params / 1e6:.2f}M':>20}")
            print(f"{'不可训练参数量':<20}{non_trainable_params:,}{'':<10}{f'{non_trainable_params / 1e6:.2f}M':>20}")
            print("=" * 60)
    ...
    def forward(self, batched_inputs):
        ...
        rgb, depth = torch.split(images.tensor, [3, 1], dim=1)
        features = self.backbone(rgb)
        features_depth = self.depth_backbone(depth)

        # features['res2'] = features['res2'] + features_depth[0]
        # features['res3'] = features['res3'] + features_depth[1]
    ...
    def instance_inference(self, mask_cls, mask_pred):
        ...
        if self.instance_on:
            ...
            for index in range(len(instances)):
                if instances[index].scores[0] > 0.8:
